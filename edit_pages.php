...
.nal {
  ...
  <body>
      ...
      <div id="board" style="width:1366px;height:768px"><?php 
        $cont=file_get_contents( $_GET['source']);
        echo $cont;
        ?></div>
       <script language="javascript">
        ...
        YUI().use('dd-drag', 'node');
            ...
            var str=window.prompt("Save as: ","filename");
            var my_header=$("head").html();
            var wait_upload=my_header+$("#board").html();
            $.post("upload.php",{page:wait_upload,name:str},function(data){
              if(data==1)window.open(str+".html","","width=600,height=800");
              else if(data==0)window.open("error.html","","width=600,height=800");
              else alert("this no science!");});
        });
      </script>
  </body>
</html>
